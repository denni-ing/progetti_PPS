...
public:
  ...
  inline T getElement(const int i, const int j) const {return matrix[i][j];}
  inline int getN() const {return N;}
  inline int getM() const {return M;}
  inline void setElement(const int i, const int j, const int val) {matrix[i][j] = val;}
...
int main(){
  ...
  const Matrix<float> m3 = Matrix<float>::sumNP(m1, m2);
  std::cout << "sum m1 + m2: " << std::endl;
  m3.printMatrix();
  
  const Matrix<float> m4 = Matrix<float>::prodMatrix(m1, m2);
